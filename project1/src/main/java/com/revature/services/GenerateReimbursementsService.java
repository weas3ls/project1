...
public class GenerateReimbursementsService {
    ...
    public static void main(String[] args) throws JsonParseException, JsonMappingException, IOException {
        ...
        Reimbursement testReimb = reimbursementService.getTicketById(15); // Should contain state from entry in
                                                                          // ers_reimbursement with id = 15

        System.out.println("ID, Amount, Description, requester ID, status ID, type ID");
        System.out.println(testReimb); // Expected: For information to match that of that in ers_reimbursement table in
                                       // SQL PostgreSQL
